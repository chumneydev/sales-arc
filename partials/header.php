<!-- contact -->
<div id="contact">
    <p><i class="fas fa-phone-alt"></i> Call Us: 864-555-1234</p>
</div>
<!-- contact -->

<!-- header -->
<div id="header" class="flex">
    <div id="logo">
        <img src="dist/images/logo.png" />
    </div>

    <div id="nav">
        <ul>
            <li><a href="./entrance-modal.php">Entrance Modal</a></li>
            <li><a href="./exit-modal.php">Exit Modal</a></li>
            <li><a href="./scratch-off-modals">Scratch-off Modals</a></li>
            <li><a href="./video-modals.php">Video Modals</a></li>
            <li><a href="./hover-buttons.php">Hover Buttons</a></li>
            <li><a href="./insertion-buttons.php">Insertion Buttons</a></li>
            <li><a href="./database-email-campaigns.php">Database Email Campaigns</a></li>
            <li><a href="./inventory.php">Inventory</a></li>
        </ul>
    </div>
</div>
    <!-- header -->